...
/* Panels pull straight from content.js's treatments entries for
   dental-implants / root-canal, so the positioning copy and the grid
   card below it never drift out of sync — single source, CONVENTIONS §3. */

const pillarsGrid = document.getElementById("pillarsGrid");
if (pillarsGrid) {
  const pillarSlugs = ["dental-implants", "root-canal"];
  const pillarLabel = { "dental-implants": "Restorative & Implants", "root-canal": "Endodontics & Pain Relief" };
...
function treatmentCardHtml(t, hrefPrefix, delay) {
  return `<a class="treatment-card" href="${hrefPrefix}${t.slug}.html" data-reveal style="transition-delay: ${delay};">
    <h3 class="treatment-card__title">${t.name}</h3>
    <p class="treatment-card__desc">${t.summary}</p>
    <div class="treatment-card__footer">
      <span class="chip treatment-card__chip">${t.sittings}</span>
...
/* Principal doctor (isPrincipal) renders the wider --principal card variant with
  ...
  doctorsGrid.innerHTML = doctors
    .map((d, i) => {
      ...
      const photoHtml = d.photo
        ? `<img src="${d.photo}" alt="Portrait of ${d.name}" loading="lazy" width="400" height="500" />`
        : `<span class="mono-label">Photo: ${d.name}</span>`;
      return `<article class="doctor-card${variant}" data-reveal style="transition-delay: ${delay};">
        <div class="doctor-card__photo" role="img" aria-label="Portrait of ${d.name}">
          ${photoHtml}
        </div>
  ...
  equipmentGrid.innerHTML = equipment
    .map((e, i) => {
      ...
      const photoHtml = e.photo
        ? `<img src="${e.photo}" alt="${e.name}" loading="lazy" width="640" height="480" />`
        : `<span class="mono-label">Photo: ${e.name}</span>`;
      return `<div class="equipment-card" data-reveal style="transition-delay: ${delay};">
        <div class="equipment-card__photo" role="img" aria-label="${e.name}">
          ${photoHtml}
        </div>
...
if (activeTreatment) {
  ...
  if (credentialName && performingDoctor) {
    ...
    const credPhoto = document.querySelector(".credential-block__photo");
    if (credPhoto && performingDoctor.photo) {
      credPhoto.classList.remove("placeholder-block");
      credPhoto.innerHTML = `<img src="${performingDoctor.photo}" alt="Portrait of ${performingDoctor.name}" loading="lazy" width="300" height="375" />`;
    } else {
      const label = document.getElementById("credentialPhotoLabel");
      if (label) label.textContent = `Photo: ${performingDoctor.name}`;
    }
  }
...
if (treatmentsHubGrid) {
  const groups = [
    { label: "Dental Implants & Restorations", slugs: ["dental-implants", "crowns-and-bridges"] },
    { label: "Endodontics & Conservative Care", slugs: ["root-canal", "dental-restoration"] },
    { label: "Periodontal & Preventive Care", slugs: ["gum-treatment"] },
    { label: "Orthodontics & Aesthetics", slugs: ["braces-and-aligners", "teeth-whitening"] },
    { label: "Paediatric Dentistry", slugs: ["kids-dentistry"] },
  ];
...
if (activeDoctor) {
  ...
  const doctorHeroPhoto = document.querySelector(".doctor-hero__photo");
  if (doctorHeroPhoto && d.photo) {
    doctorHeroPhoto.classList.remove("placeholder-block");
    doctorHeroPhoto.innerHTML = `<img src="${d.photo}" alt="Portrait of ${d.name}" loading="eager" width="400" height="500" />`;
  } else {
